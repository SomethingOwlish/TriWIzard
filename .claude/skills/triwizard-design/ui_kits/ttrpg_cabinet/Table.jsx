...
(function () {
  ...
  const { Button, IconButton, Card, Badge, Tag, Avatar, Tabs, DiceRoller, StatBlock, DataTable, Timeline, CommentThread, Switch, ThemeSwitcher } = DS;
  ...
  const Dice = (p) => <Ico {...p}><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8" cy="8" r="1.2" fill="currentColor"/><circle cx="16" cy="16" r="1.2" fill="currentColor"/><circle cx="12" cy="12" r="1.2" fill="currentColor"/></Ico>;
  const Chart = (p) => <Ico {...p}><path d="M3 3v18h18"/><path d="M7 14l3-3 3 2 4-6"/></Ico>;
  const Grid = (p) => <Ico {...p}><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></Ico>;
  const Clock = (p) => <Ico {...p}><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></Ico>;
  const Card2 = (p) => <Ico {...p}><rect x="4" y="3" width="16" height="18" rx="2"/><path d="M8 8h8M8 12h6"/></Ico>;
  ...
  function TopBar({ master, setMaster, theme, setTheme }) {
    return (
      <div style={{ height: 60, flexShrink: 0, borderBottom: '1px solid var(--border-1)', background: 'color-mix(in srgb, var(--surface-page) 88%, transparent)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', padding: '0 24px', gap: 16 }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 18 }}>
          ...
          <ThemeSwitcher scope="ttrpg" applyTo="document" value={theme} onChange={setTheme} />
        </div>
      </div>
    );
  }
    ...
    return (
      <div style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <TopBar master={master} setMaster={setMaster} theme={theme} setTheme={setTheme} />
  ...
  window.TtrpgCabinet = { Table, NavRail, TopBar, PlayerCard, DiceView, Graphs, Tables, Chronicle, MasterScreen };
})();
