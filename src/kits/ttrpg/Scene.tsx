/* TriWizard — TTRPG live Scene (Tier 3: B3.1 setup, B3.2 live sync, B3.3 dice).
   The one live surface in the app. Everyone at the table shares a single
   onSnapshot on sceneTable/main: the active scene, its NPC tokens, the
   participants with their scene-local condition trackers, the conflict turn
   order, and each participant's last roll. Players roll 2d6 + stat (auto-pulled
   from their card and adjusted by the scene-local conditions) and resolve on the
   Durmstrang ladder; the GM stages scenes, seats participants, toggles harm, and
   calls initiative. Older rolls live in a shared stream read on demand. */
...
/** The effective modifier for a stat: base + scene-local condition penalty,
 *  honouring Critical state (−6 to all rolls; −10 to stats already hit). */
...
/** Build a fresh participant from a published (or drafted) character sheet. */
function participantFromChar(rec: CharacterRecord): SceneParticipant {
  const sheet = rec.published ?? rec.draft;
  const stats = sheet?.stats ?? { dex: 0, end: 0, will: 0, mag: 0, per: 0, wit: 0 };
  const conditions = sheet?.conditions ?? { ...NO_COND };
  return {
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ ...mono, fontSize: 9, marginTop: 3 }}>
          {roll.dice[0]}+{roll.dice[1]} {signed(roll.mod)}{roll.moveName ? ` · ${roll.moveName}` : roll.stat ? ` · ${STATS[roll.stat].en}` : ''}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
      {STAT_KEYS.map((k) => {
        ...
        return (
          <button key={k} type="button" onClick={() => { const i = cycle.indexOf(lvl); onChange({ ...p.conditions, [k]: cycle[(i + 1) % 3] }); }}
  ...
  return (
    <Card padding="14px" accentEdge={acting} style={p.neutralized ? { opacity: 0.6 } : undefined}>
      ...
      <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
        {STAT_KEYS.map((k) => (
          <span key={k} style={{ ...mono, fontSize: 9, padding: '2px 6px', borderRadius: 'var(--radius-xs)', background: 'var(--surface-inset)', color: 'var(--text-2)' }}>
            {STATS[k].en.slice(0, 3)} {signed(p.stats[k] ?? 0)}
          </span>
        ))}
      </div>
      <div style={{ marginTop: 8 }}>
        <ConditionPips p={p} editable={gm} onChange={onConditions} />
      </div>
...
function RollPanel({ mine, moves, sceneName, onRolled }: {
  ...
  async function go() {
    ...
    try {
      ...
      onRolled(e instanceof Error ? e.message : 'The bones would not fall.', 'dead');
    } finally { setBusy(false); }
  }

  if (mine.length === 0) {
    return (
      <Card title="Cast the bones" eyebrow="2d6 + stat" padding="18px">
        <div style={{ ...serif, fontStyle: 'italic', color: 'var(--text-3)' }}>You hold no character in this scene. The master must seat you first.</div>
      </Card>
    );
  }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {mine.length > 1 && (
          <Field label="Acting as">
            <Select value={pk} onChange={(e) => setPk(e.target.value)}>
              {mine.map((m) => <option key={m.key} value={m.key}>{m.name}</option>)}
            </Select>
          </Field>
        )}
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <Field label="Stat" style={{ flex: 1 }}>
            <Select value={effStat} disabled={!!move?.stat} onChange={(e) => setStat(e.target.value as StatKey)}>
              {STAT_KEYS.map((k) => <option key={k} value={k}>{STATS[k].en} ({STATS[k].ru})</option>)}
            </Select>
          </Field>
          <Field label="Situational" style={{ width: 110 }}>
            <Input type="number" value={String(situational)} onChange={(e) => setSituational(Number(e.target.value) || 0)} />
          </Field>
        </div>
        {calc && (
          <div style={{ ...mono, fontSize: 10, color: 'var(--text-2)' }}>
            2d6 · base {signed(calc.base)}
            {calc.pen !== 0 && <span style={{ color: 'var(--status-wounded)' }}> · {calc.crit ? 'critical' : 'condition'} {signed(calc.pen)}</span>}
            {situational !== 0 && ` · situational ${signed(situational)}`}
            {' = '}<span style={{ color: 'var(--accent-text)' }}>roll {signed(calc.mod)}</span>
          </div>
        )}
    ...
    <Dialog open width={600} onClose={onClose} eyebrow="Master · stage a scene" title={s.name || 'A new scene'}
      ...
      <div style={{ display: 'grid', gap: 14, padding: '4px 0' }}>
        ...
        <div>
          ...
          <NpcEditor npcs={s.npcs} onChange={(n) => set({ npcs: n })} />
        </div>
      </div>
    </Dialog>
  );
...
function LogDialog({ onClose }: { onClose: () => void }) {
  ...
  React.useEffect(() => { fetchRollLog(60).then(setRows).catch(() => setRows([])); }, []);
  return (
    <Dialog open width={560} onClose={onClose} eyebrow="The shared stream" title="Roll log"
...
export function SceneModule({ role, userUid }: { role: CardRole; userUid: string | null }) {
  ...
  const [table, setTable] = React.useState<SceneTable | null>(null);
  ...
  React.useEffect(() => watchSceneTable(setTable), []);
  ...
  // ---- GM mutations on the live table (whole-array writes) ----
  ...
  function rollInitiativeAll() {
    ...
    const scored = participants.map((p) => ({ key: p.key, total: rollInitiative().total }));
  ...
  async function persistScene(input: SceneInput) {
    try {
      if (editing?.id) await saveScene(editing.id, input); else await createScene(input);
      setEditing(null); setToast({ tone: 'accent', msg: 'The scene is staged.' });
    } catch (e) { setToast({ tone: 'dead', msg: e instanceof Error ? e.message : 'The scene would not hold.' }); }
  }

  if (!t) return <div style={{ padding: 40, ...mono }}>Drawing back the curtain…</div>;
    ...
    <div style={{ padding: 32, maxWidth: 1180, margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 20, gap: 16, flexWrap: 'wrap' }}>
        <div>
          ...
          <h1 style={{ margin: '4px 0 0', ...display, fontSize: 34 }}>{t.scene?.name || 'No scene is set'}</h1>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 20, alignItems: 'start' }}>
        {/* Left — participants & turn */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          {t.turn && (
            <Card title="The cycle" eyebrow={`Round ${t.turn.round} · initiative`} padding="14px"
              actions={gm ? <Button size="sm" variant="secondary" onClick={advanceTurn}>Next turn →</Button> : null}>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                {t.turn.order.map((k, i) => {
                  const p = participants.find((x) => x.key === k); if (!p) return null;
                  return <Badge key={k} tone={i === t.turn!.actingIndex ? 'accent' : 'neutral'} dot={i === t.turn!.actingIndex}>{p.name}</Badge>;
                })}
              </div>
            </Card>
          )}
          ...
          <Card title="At the scene" eyebrow={`${participants.length} present`} padding="14px"
            actions={gm ? (
              <div style={{ display: 'flex', gap: 6 }}>
                <Button size="sm" variant="ghost" iconStart={<Bolt s={14} />} onClick={rollInitiativeAll}>Initiative</Button>
                <Button size="sm" variant="ghost" iconStart={<Plus s={14} />} onClick={addNpc}>NPC</Button>
              </div>
            ) : null}>
            {participants.length === 0 ? (
              <div style={{ ...serif, fontStyle: 'italic', color: 'var(--text-3)' }}>No one stands in the scene yet.</div>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 12 }}>
                {participants.map((p) => (
                  <ParticipantCard key={p.key} p={p} roll={t.lastRolls[p.key]} gm={gm} acting={p.key === actingKey}
                    onConditions={(c) => patchParticipant(p.key, { conditions: c })}
                    onNeutralize={(v) => patchParticipant(p.key, { neutralized: v })}
            ...
            {gm && seatable.length > 0 && (
              <div style={{ marginTop: 14, paddingTop: 14, borderTop: '1px solid var(--border-1)' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {seatable.map((c) => (
                    <Button key={c.id} size="sm" variant="secondary" iconStart={<Plus s={13} />} onClick={() => addPc(c)}>{c.name || 'Unnamed'}</Button>
                  ))}
                </div>
              </div>
            )}
          ...
          {visibleNpcs.length > 0 && (
            <Card title="On the scene" eyebrow="NPCs" padding="14px">
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {visibleNpcs.map((n) => (
                  <div key={n.id} style={{ display: 'flex', alignItems: 'center', gap: 8, paddingBottom: 8, borderBottom: '1px solid var(--border-1)' }}>
                    <span style={{ ...serif }}>{n.name}</span>
                    {!n.shown && gm && <Badge tone="wounded" outline>hidden</Badge>}
                    {n.note && <span style={{ ...mono, fontSize: 9 }}>{n.note}</span>}
                  </div>
                ))}
              </div>
            </Card>
        ...
        {/* Right — roll panel + (GM) library */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <RollPanel mine={mine} moves={moves} sceneName={t.scene?.name ?? ''} onRolled={(msg, tone) => setToast({ tone: tone === 'dead' ? 'dead' : 'accent', msg })} />

          {gm && (
            <Card title="Scene library" eyebrow="Stage & switch" padding="14px"
              actions={<Button size="sm" variant="ghost" iconStart={<Plus s={14} />} onClick={() => setEditing({ id: null, input: blankScene() })}>New</Button>}>
              {library.length === 0 ? (
                <div style={{ ...serif, fontStyle: 'italic', color: 'var(--text-3)' }}>No scenes staged yet.</div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {library.map((s) => {
                    const active = s.id === t.activeSceneId;
                    return (
                      <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 'var(--radius-sm)', border: `1px solid ${active ? 'var(--accent)' : 'var(--border-1)'}`, background: active ? 'var(--accent-soft)' : 'var(--surface-inset)' }}>
                        <div style={{ flex: 1, minWidth: 0 }}>
      ...
      {editing && (
        <SceneEditor initial={editing.input} onSave={persistScene} onClose={() => setEditing(null)}
          onDelete={editing.id ? async () => {
            const id = editing.id!;
            try { await deleteScene(id); if (t.activeSceneId === id) await updateTable({ activeSceneId: null, scene: null, npcs: [] }); setEditing(null); }
            catch { /* noop */ }
          } : undefined} />
      )}
      ...
      {toast && (
        <div style={{ position: 'fixed', right: 24, bottom: 24, zIndex: 1200 }}>
          <Toast tone={toast.tone} title="The scene" onDismiss={() => setToast(null)}>{toast.msg}</Toast>
        </div>
      )}
    </div>
  );
