...
function BarChart({ data, max, height = 180 }: BarChartProps) {
...
function LineChart({ points, height = 180 }: LineChartProps) {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 20, alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <Card title="Vitals" padding="20px">
            <div style={{ display: 'flex', gap: 18 }}>
              {([['Hit Points', '21 / 24', 0.875], ['Will', '9 / 10', 0.9], ['Dread', '4 / 10', 0.4]] as [string, string, number][]).map(([l, v, p]) => (
        ...
        <Card title="Spells & Rites" eyebrow="Known" padding="20px">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {([['Summon the Drowned', 'd8'], ['Salt Ward', 'd4'], ['Read the Bones', '—'], ['Cold Whisper', 'd6']] as [string, string][]).map(([s, d]) => (
...
function DiceView() {
  const [log, setLog] = React.useState<Roll[]>([{ sides: 20, raw: 14, total: 17 }, { sides: 6, raw: 5, total: 5 }]);
  return (
    <div style={{ padding: 32, maxWidth: 1080, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, alignItems: 'start' }}>
        <DiceRoller defaultSides={20} modifier={3} onRoll={(r: Roll) => setLog((l) => [r, ...l].slice(0, 8))} />
    ...
    <div style={{ padding: 32, maxWidth: 1080, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        <div>
          ...
          <DataTable dense
            columns={[{ key: 'roll', label: 'Roll', mono: true, width: 70, align: 'right' }, { key: 'effect', label: 'Effect' }, { key: 'sev', label: 'Severity', render: (v: string) => <Badge tone={v === 'Grave' ? 'dead' : v === 'Ill' ? 'wounded' : 'alive'} dot>{v}</Badge> }]}
            rows={[
        ...
        <div>
          ...
          <DataTable
            columns={[
              { key: 'name', label: 'Character', render: (v: React.ReactNode, r: { i: string; s: 'alive' | 'wounded' | 'dead' }) => <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}><Avatar initials={r.i} size="xs" status={r.s} />{v}</span> },
    ...
    <div style={{ padding: 32, maxWidth: 1080, margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: 20, alignItems: 'start' }}>
        <Card title="Initiative" eyebrow="This encounter" padding="0">
          <div>
            {([['Sieglinde Frost', 21, 'SF', 'alive'], ['Kára Nightveil', 17, 'KN', 'alive'], ['The Drowned Host', 14, 'DH', 'wounded'], ['Ivar the Drowned', 9, 'IV', 'wounded']] as [string, number, string, 'alive' | 'wounded' | 'dead'][]).map(([n, init, i, s], idx) => (
