/* TriWizard — LARP public site.
   The open, no-login marketing & lore surface. Dark Scandinavian academy
   with a folklore feel; only the `dark` and `light` skins (blood accent). */
import React from 'react';
import { Button, Card, Badge, Tag, ThemeSwitcher, Avatar } from '../components';
import { Anchor, Scroll, Compass, Shield } from './icons';

type Page = 'home' | 'lore' | 'rules' | 'order';
...
const rune: React.CSSProperties = { fontFamily: 'var(--font-display)', color: 'var(--accent-text)', letterSpacing: '0.3em' };
...
function Header({ page, onNav, onEnterCabinet, theme, setTheme }: HeaderProps) {
  const links: [Page, string][] = [['lore', 'The Keep'], ['rules', 'The Rite'], ['order', 'The Order']];
  return (
    <header style={{ position: 'sticky', top: 0, zIndex: 100, background: 'color-mix(in srgb, var(--surface-page) 86%, transparent)', backdropFilter: 'blur(10px)', borderBottom: '1px solid var(--border-1)' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: '0 32px', height: 68, display: 'flex', alignItems: 'center', gap: 32 }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <Button variant="secondary" size="sm" onClick={onEnterCabinet}>Enter the Cabinet</Button>
        </div>
      </div>
    </header>
  );
  ...
  return (
    <section style={{ position: 'relative', overflow: 'hidden', borderBottom: '1px solid var(--border-1)' }} className="tw-stone-wash">
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: '110px 32px 96px' }}>
        ...
        <h1 style={{ margin: 0, fontFamily: 'var(--font-display)', fontWeight: 900, fontSize: 'clamp(48px, 7vw, 92px)', lineHeight: 0.98, letterSpacing: '-0.01em', color: 'var(--text-1)', maxWidth: '16ch' }}>
        ...
        <div style={{ display: 'flex', gap: 14, marginTop: 38, flexWrap: 'wrap' }}>
          ...
          <Button size="lg" variant="secondary" iconStart={<Scroll s={18} />} onClick={() => onNav('rules')}>Read the Rite</Button>
        </div>
...
interface Pillar {
  icon: (p: { s?: number; style?: React.CSSProperties }) => React.ReactElement;
  t: string;
  b: string;
}
...
function Landing({ onNav, onTTRPG }: LandingProps) {
  return (
    <div>
      <Hero onNav={onNav} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 48, alignItems: 'center' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              ...
              <Button variant="ghost" onClick={onTTRPG}>The TTRPG table →</Button>
            </div>
          </div>
...
function RulesPage() {
  const [tab, setTab] = React.useState<'crossing' | 'oath' | 'standing'>('crossing');
  const bodyByTab: Record<'crossing' | 'oath' | 'standing', [string, string]> = {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 40, alignItems: 'start' }}>
        <div style={{ position: 'sticky', top: 92 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {([['crossing', 'I · The Crossing'], ['oath', 'II · The Iron Oath'], ['standing', 'III · House Standing']] as [typeof tab, string][]).map(([k, l]) => (
...
function OrderPage() {
  const masters: [string, string, string][] = [
...
/** LarpSite — the public site shell. Switches pages from the header nav. */
export default function LarpSite({ onEnterCabinet, onTTRPG, theme, setTheme }: Props) {
  const [page, setPage] = React.useState<Page>('home');
  const map: Record<Page, React.ReactNode> = {
    home: <Landing onNav={setPage} onTTRPG={onTTRPG} />,
    lore: <Landing onNav={setPage} onTTRPG={onTTRPG} />,
    rules: <RulesPage />,
    order: <OrderPage />,
  };
  return (
    <div>
      <Header page={page} onNav={setPage} onEnterCabinet={onEnterCabinet} theme={theme} setTheme={setTheme} />
      <main>{map[page] || map.home}</main>
      <Footer />
    </div>
  );
}

export { Header, Footer, Landing, RulesPage, OrderPage };
