...
- [ ] **B4.5** **Session planner** (calendar, like the Notion sessions database). GM announces sessions on a month/agenda calendar; each session carries a title, in-world + real date, format (Online/Offline/Mixed/Individual), status (Announced/Closed/Played), a short pitch, and a signup roster (players enrol; GM confirms, cap ≤ 6). Cross-links the world Chronology (a played session ⇒ a timeline event) and reuses the save→publish + roster patterns. New collection `sessions/{id}`. _Requested 2026-07-01 alongside the Notion Tier-4 import._

> **Notion import (2026-07-01).** The public TriWizard Notion workspace was crawled and stored as typed seed data in `src/data/notion/*` (lore tree, rule-helper articles, world chronology, 51 NPC dossiers, and — kept strictly separate — 20 player rows). A GM-only "Import from Notion" button in each Tier-4 module (`src/lib/notionSeed.ts`) pours the seed into the live collections as **unpublished drafts** for review. **Players are never imported into the bestiary** (split by the Notion `Игрок` field). Portraits/covers are stored as Notion public image **links** (proxy re-signs them), not copied. Seed loads lazily (dynamic import ⇒ own bundle chunk).
